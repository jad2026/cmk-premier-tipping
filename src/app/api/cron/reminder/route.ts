...
export async function GET(request: Request) {
  ...
  for (const gw of gws) {
    ...
    if (hoursUntilDeadline < 0 || hoursUntilDeadline > HOURS_BEFORE_DEADLINE) {
      console.log(
        `[reminder] ${gw.label} deadline is ${hoursUntilDeadline.toFixed(1)}h away — outside ${HOURS_BEFORE_DEADLINE}h window`
      );
      continue;
    }
    ...
    for (let i = 0; i < userSlice.length; i += BATCH) {
      ...
      for (let j = 0; j < settled.length; j++) {
        ...
        if (ok) {
          sent++; totalSent++;
          console.log(`[reminder] Sent 24h reminder to ${batch[j].email} for ${competitionName} ${gw.label} (${batch[j].picksCount}/${totalFixtures} picks)`);
        } else {
          failed++; totalFailed++;
        }
      }
